  ...
  return !window.BDFDB_Global ||
    ...
    ? class {
      ...
      load() {
        ...
        if (!window.BDFDB_Global.pluginQueue.includes(this.name)) {
          window
            .BDFDB_Global.pluginQueue.push(this.name);
        }
      }
      start() {
        this.load();
      ...
      getSettingsPanel() {
        ...
        template.innerHTML =
          `<div style="color: var(--header-primary); font-size: 16px; font-weight: 300; white-space: pre; line-height: 22px;">The Library Plugin needed for ${this.name} is missing.\nPlease click <a style="font-weight: 500;">Download Now</a> to install it.</div>`;
        template.content.firstElementChild.querySelector("a").addEventListener(
          "click",
          this.downloadLibrary,
        );
    ...
    : (([Plugin, BDFDB]) => {
      ...
      return class SplitLargeMessages extends Plugin {
          ...
          this.defaults = {
            general: {
              byNewlines: {
                value: false,
                description:
                  "Try to split Messages on Newlines instead of Spaces",
                note:
                  "This will stop Sentences from being cut, but might result in more Messages being sent",
              },
            },
            amounts: {
              splitCounter: {
                value: 0,
                description:
                  "Messages will be split after roughly X Characters",
              },
              maxMessages: {
                value: 0,
                description: "Maximum Number of split Messages",
                note:
                  "(0 for unlimited) Messages beyond this count will be discarded",
              },
            },
          };
        ...
        onStart() {
          maxMessageLength =
            BDFDB.LibraryModules.NitroUtils.canUseIncreasedMessageLength(
                BDFDB.LibraryStores.UserStore.getCurrentUser(),
              )
              ? BDFDB.DiscordConstants.MAX_MESSAGE_LENGTH_PREMIUM
              : BDFDB.DiscordConstants.MAX_MESSAGE_LENGTH;

          BDFDB.PatchUtils.patch(
            this,
            BDFDB.LibraryModules.ChatRestrictionUtils,
            "applyChatRestrictions",
            {
              before: (e) => {
                if (
                  e.methodArguments[0] && e.methodArguments[0].content &&
                  !this.isSlowDowned(e.methodArguments[0].channel)
                ) e.methodArguments[0].content = "_";
              },
            },
          );
              ...
              settingsItems.push(
                BDFDB.ReactUtils.createElement(
                  BDFDB.LibraryComponents.SettingsSaveItem,
                  {
                    ...
                    value: this.settings.amounts.splitCounter < 1000 ||
                        this.settings.amounts.splitCounter > maxMessageLength
                      ? maxMessageLength
                      : this.settings.amounts.splitCounter,
                  },
                ),
              );
        ...
        processChannelTextAreaContainer(e) {
          if (
            e.instance.props.type !=
              BDFDB.DiscordConstants.ChannelTextAreaTypes.NORMAL &&
            e.instance.props.type !=
              BDFDB.LibraryComponents.ChannelTextAreaTypes.SIDEBAR
          ) return;
          const splitMessageLength =
            this.settings.amounts.splitCounter < 1000 ||
              this.settings.amounts.splitCounter > maxMessageLength
              ? maxMessageLength
              : this.settings.amounts.splitCounter;
          if (!e.returnvalue) {
            BDFDB.PatchUtils.patch(this, e.instance.props, "onSubmit", {
              instead: (e2) => {
                if (
                  e2.methodArguments[0].value.length > splitMessageLength &&
                  !this.isSlowDowned(e.instance.props.channel)
                ) {
                  e2.stopOriginalMethodCall();
                  let messages = this.formatText(e2.methodArguments[0].value)
                    .filter((n) => n);
                  for (let i in messages) {
                    BDFDB.TimeUtils.timeout((_) => {
                      ...
                      e2.originalMethod(
                        last
                          ? Object.assign({}, e2.methodArguments[0], {
                            value: messages[i],
                          })
                          : { stickers: [], uploads: [], value: messages[i] },
                      );
                      if (i >= messages.length - 1) {
                        BDFDB.NotificationUtils.toast(
                          this.labels.toast_allsent,
                          { type: "success" },
                        );
                      }
            ...
            let [children, index] = BDFDB.ReactUtils.findParent(e.returnvalue, {
              name: "ChannelTextAreaCounter",
            });
            if (
              index > -1 && children[index].props.textValue &&
              children[index].props.textValue.length > splitMessageLength &&
              !this.isSlowDowned(e.instance.props.channel)
            ) {
              children[index] = BDFDB.ReactUtils.createElement("div", {
                className: BDFDB.disCNS.textareacharcounter +
                  BDFDB.disCN.textareacharcountererror,
                children: BDFDB.ReactUtils.createElement(
                  BDFDB.LibraryComponents.TooltipContainer,
                  {
                    text: Math.ceil(
                      children[index].props.textValue.length /
                        splitMessageLength * (39 / 40),
                    ) + " " + BDFDB.LanguageUtils.LanguageStrings.MESSAGES,
                    children: BDFDB.ReactUtils.createElement("span", {
                      children: splitMessageLength -
                        children[index].props.textValue.length,
                    }),
                  },
                ),
        ...
        processChannelTextAreaEditor(e) {
          if (
            ...
            e.instance.props.type ==
              BDFDB.LibraryComponents.ChannelTextAreaTypes.SIDEBAR
          ) e.instance.props.uploadPromptCharacterCount = 100000;
        }
        ...
        formatText(text) {
          ...
          if (longWords) {
            for (let longWord of longWords) {
              ...
              longWord.split("").forEach((c) => {
                if (
                  shortWords[count1] &&
                  (shortWords[count1].length >=
                      splitMessageLength * (19 / 20) ||
                    (c == "\n" &&
                      shortWords[count1].length >=
                        splitMessageLength * (19 / 20) - 100))
                ) count1++;
                shortWords[count1] = shortWords[count1]
                  ? shortWords[count1] + c
                  : c;
              });
          ...
          text.split(separator).forEach((word) => {
            if (
              messages[count2] &&
              (messages[count2] + "" + word).length >
                splitMessageLength * (39 / 40)
            ) count2++;
            messages[count2] = messages[count2]
              ? messages[count2] + separator + word
              : word;
          });
          ...
          for (let j = 0; j < messages.length; j++) {
            ...
            let codeLines = messages[j].match(
              /[^`]{0,1}`{1,2}[^`]|[^`]`{1,2}[^`]{0,1}/gm,
            );
            ...
            if (codeBlocks && codeBlocks.length % 2 == 1) {
              ...
              insertCodeLine = codeLines[codeLines.length - 1].replace(
                /[^`]/g,
                "",
              );
          ...
          return this.settings.amounts.maxMessages
            ? messages.slice(0, this.settings.amounts.maxMessages)
            : messages;
        }
