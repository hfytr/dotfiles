      ...
      const plugin = (Plugin, Library) => {
        ...
        const createElement = (type, props, ...children) => {
          if (typeof type === "function") {
            return type({ ...props, children: [].concat() });
          }
        ...
        class Tooltip {
          containerClassName = Utilities.className(
            "FileViewerTooltip",
            ...["tooltip", "tooltipTop", "tooltipPrimary"].map((c) =>
              DiscordClasses.Tooltips?.[c]?.value
            ),
          );
          ...
          constructor(target, { text, spacing }) {
            ...
            this.tooltip = createElement("div", {
              ...
              children: [
                createElement("div", {
                  className: this.pointerClassName,
          ...
          show() {
            ...
            let left = targetRect.x + targetRect.width / 2 -
              tooltipRect.width / 2;
        ...
        class FileViewerButton {
          ...
          render() {
            ...
            if (!this.fileViewRef && this.open) {
              const embed = htmlToElement(
                `<iframe class="FileViewerEmbed" src="${this.url}" width="450" height="500"></iframe>`,
              );

              this.target.closest(`.${Classes.messageAttachment}`).parentNode
                .appendChild(embed);
        ...
        return class FileViwer extends Plugin {
          ...
          addViewerButton(attachmentElement) {
            ...
            let useGoogleProvider = this.settings.forceProvider ||
              isGoogleExtension;
            ...
            if (fileTooBig) {
              ...
              new Tooltip(warningIcon, {
                text:
                  "This file is over 10mb! FileViewer might not be able to preview this file.",
                spacing: 8,
              });
            }
          }
          ...
          observer({ addedNodes }) {
            for (const added of addedNodes) {
              ...
              const attachmentElement = added.getElementsByClassName(
                Classes.attachment,
              );
              if (attachmentElement.length > 0) {
                Array.from(attachmentElement).forEach((a) => {
                  if (a.getElementsByClassName("FileViewerButton").length > 0) {
                    return;
                  }
